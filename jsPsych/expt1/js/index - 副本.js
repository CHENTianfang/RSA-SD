...
let jsPsych = initJsPsych({
    use_webaudio: true,
    on_finish: function () {
        jsPsych.data.get().localSave('csv', '实验结果.csv');
    }
});
...
let instruction_consent = {
    ...
    pages: function () {
        ...
        <div class="consent-container" style="width: 80%; margin: 0 auto; text-align: left;">
            ...
            <p>您可能需要执行的任务包括以下一项或多项，预计总实验时长约为12-15分钟：</p>
            <ol>
...
var ambiguousTourismQuiz = {
    ...
    on_finish: function (data) {
        if (data.response.Q0 == "告诉游客有关蘑菇特征的事实" && data.response.Q1 == "它们可以吃" && data.response.Q2 == `斑点价值${typeScores.Spotted}分`) {
        ...
        loop_function: function () {
            if (repetition < 2) {
                if (TourismQuizPass && colorQuizPass && textureQuizPass && generalQuizPass && specificQuizPass) {
                    ...
                    jsPsych.addNodeToEndOfTimeline(block1);
                    jsPsych.addNodeToEndOfTimeline(rest);
                    jsPsych.addNodeToEndOfTimeline(block2);
...
var trialCore = {
    ...
    html: function () {
        featureList.shuffle()
        valueList.shuffle()
        listenerName = jsPsych.timelineVariable('listenerName')
        avatarImg = "asset/img/people/" + listenerName + ".png"
        msrList = jsPsych.timelineVariable('msrList')
        // let msrValueList = []
        // for (let i = 0; i < 3; i++) {
        //     msrImgList.push("asset/img/mushrooms/" + jsPsych.timelineVariable('msrList')[i].msr + ".png")
        //     msrValueList.push(jsPsych.timelineVariable('msrList')[i].value)
        // }
...
let rest = {
    ...
    stimulus: `
    <div class="break-container">
        <p class="break-message">☕️&nbsp实验已经过半，请耐心地休息一会儿。</p>
