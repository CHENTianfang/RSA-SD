...
def check_group(group_start, group_end):
    ...
    group_data = data.iloc[group_start:group_end, [17, 18]]
    group_r = group_data.iloc[:, 0]
    group_s = group_data.iloc[:, 1]
    
    # 统计 R 列和 S 列相等的次数
    equal_count = (group_r == group_s).sum()
    
    # 输出该组的检查结果
    print(f"第{group_start + 2}-{group_end + 2}行相等的次数: {equal_count}")
...
def check_non_empty_rows():
    ...
    for idx, row in data.iterrows():
        ...
        if pd.notna(row[17]) and pd.notna(row[18]):  # 检查非NaN值
            non_empty_rows.append((idx + 2, row[17], row[18]))
