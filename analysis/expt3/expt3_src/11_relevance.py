...
def compute_relevance_utility(mushrooms, utterance):
    ...
    try:
        ...
        if not (-3 <= feature_value <= 3):
            return np.nan

        # 概率分配计算
        P_alloc = (feature_value - (-3)) / (3 - (-3))
...
# 数据解析模块
# ==================================================================
def parse_mushroom(cell, value_choice='平均'):
    """解析蘑菇数据"""
    try:
        ...
        # 选择使用的 value
        if value_choice == 'A1':
            value = int(data["value_A1"])
        elif value_choice == 'A2':
            value = int(data["value_A2"])
        else:  # 默认使用 (A1 + A2) / 2
            value = (int(data["value_A1"]) + int(data["value_A2"])) / 2
...
# 文件处理模块
# ==================================================================
def process_single_file(input_path, output_path, value_choice='平均'):
    """处理单个CSV文件"""
    try:
        with open(input_path, "r", encoding="utf-8-sig") as f_in, \
            ...
            header = next(reader)
            writer.writerow(header + ["RelevanceUtility"])
            ...
            for row in reader:
                try:
                    ...
                    # 解析蘑菇数据，传入 value_choice 参数
                    mushrooms = [parse_mushroom(cell, value_choice) for cell in row[12:15]]
                    utterance = parse_utterance(row[8])
                    utility = compute_relevance_utility(mushrooms, utterance)
                    writer.writerow(row + [f"{utility:.4f}"])
...
def process_batch_files(input_pattern, output_dir, value_choice='平均'):
    """批量处理CSV文件"""
    ...
    for path in sorted(glob.glob(input_pattern), key=os.path.getmtime):
        try:
            ...
            stats = process_single_file(path, output_path, value_choice)
            summary.append({
...
def generate_summary_report(data, output_dir):
    ...
    with open(report_path, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=["文件名", "状态", "平均效用", "有效数据量", "总行数", "错误信息"])
        writer.writeheader()
...
if __name__ == "__main__":
    value_choice = '平均'  # 可以是 'A1', 'A2', 或者 '平均'
    process_batch_files(r"C:/analysis/expt3/expt3_out/unbias/*.csv", r"C:/analysis/expt3/expt3_result/relevance/", value_choice)
